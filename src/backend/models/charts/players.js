...
module.exports = {
    ...
    async getPlayersLastMonth() {
        return queryAndCache(
            'SELECT ROUND(AVG(`players`), 0) as players, DATE_FORMAT(date,\'%m-%Y\') as month_format, MONTHNAME(date) as month_name FROM players WHERE server_online = TRUE AND date > UTC_TIMESTAMP() - INTERVAL 12 MONTH GROUP BY month_format, month_name LIMIT 30',
            "players_past_month",
            60,
            false
        )
